...
A quarter of all movement runs through **hold rooms, field offices, and staging sites** — places ICE processes people in transit. Its population reports exclude them by design, so they have no average daily population and get no circle on the map. They are drawn instead as fixed-size rings behind a legend toggle, off by default, and always as ring markers when a flow reaches one, so a route can never end at nothing. Each one has its own flow board and can be selected like a facility — AROCC in Arizona moves 38,330 stints a year to Florence Staging, Alexandria Staging, and Venezuela. `flows.endpoints` in the match report counts them.
